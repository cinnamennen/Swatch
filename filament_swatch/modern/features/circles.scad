...
module create_circle_tests() {
    ...
    attach(TOP)
    left(SHELF_WIDTH/2 - CIRCLE_DIAMETER)
    back(SHELF_HEIGHT/2 - CIRCLE_RADIUS)
    fwd(.5)
    up(P_EPSILON) {
        // Create the test holes
        for (i = [0:6]) {
            tag("remove") 
            right(i * CIRCLE_SPACING)
                cyl(d=CIRCLE_DIAMETER, h=BASE_THICKNESS*2 + P_EPSILON);
        }
        
        // Create the hemisphere test
        tag("keep") 
        right(1 * CIRCLE_SPACING)
        ...
        // Create the small cylinder test with cuboid
        tag("keep")
        right(2 * CIRCLE_SPACING)
        tag_scope("cylinder") diff("remove") {
            down(SHELF_THICKNESS/2) {
                cyl(d=2, h=SHELF_THICKNESS + P_EPSILON);
                tag("remove")
                    cyl(d=1, h=SHELF_THICKNESS + 2*P_EPSILON);
            }
