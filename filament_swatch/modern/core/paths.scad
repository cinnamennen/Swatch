...
ROUNDED_HANDLE_PATH = round_corners([for (i = HANDLE_PATH_INDICES) INNER_PATH[i]], 
                 radius=CORNER_RADIUS, $fn=SEGMENTS);
        // height

SHELF_WIDTH = abs(INNER_PATH[LEFT_TOP_CHAMFER].x - INNER_PATH[RIGHT_BOTTOM_CORNER].x);
SHELF_HEIGHT = abs(INNER_PATH[RIGHT_TOP_CORNER].y - INNER_PATH[RIGHT_BOTTOM_CORNER].y);

SHELF_CENTER=
    [INNER_PATH[LEFT_TOP_CHAMFER].x + SHELF_WIDTH/2,  
     (INNER_PATH[RIGHT_TOP_CORNER].y + INNER_PATH[RIGHT_BOTTOM_CORNER].y)/2];  