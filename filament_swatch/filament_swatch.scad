...
print_settings_top = str(layer_height[0], " ", layer_height[1], " ", layer_height[2]);
...
{            
    ...
    difference()
    {
        ...
        {
            steparea_w = calc_step_area_width(SWATCH_WIDTH, SWATCH_WALL, len(STEP_THICKNESSES), i);
            translate([SWATCH_WALL*1, SWATCH_WALL*STEP_AREA_DISTANCE, 
                      STEP_THICKNESSES[i]<0 ? STEP_BRIDGE_OFFSET : STEP_THICKNESSES[i]])
            {
                rounded_square(steparea_w, steparea_h, SWATCH_THICKNESS*2, SWATCH_ROUND);
            }
        }
        ...
        textlines(
            ...
            textsize_upper, textsize_lower, linesep, TEXT_FONT
        );
