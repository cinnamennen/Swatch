...
function calc_circle_position(circle_index) = [
  SWATCH_WALL + TEST_CIRCLE_RADIUS +
      TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING * circle_index,
  SWATCH_HEIGHT - SWATCH_WALL -
  TEST_CIRCLE_RADIUS
];
...
module create_test_circle(circle_index) {
  ...
  translate([ pos[0], pos[1], SWATCH_THICKNESS / 2 - TEST_Z_OFFSET ])
      cylinder(r = TEST_CIRCLE_RADIUS, h = SWATCH_THICKNESS + TEST_EXTRA_HEIGHT,
               center = true);
}
...
module create_filament_holder() {
  ...
  translate([
    pos[0], SWATCH_HEIGHT + TEST_Z_OFFSET + FILAMENT_HOLDER_OFFSET,
    FILAMENT_RADIUS +
    TEST_EXTRA_HEIGHT
  ]) rotate([ 90, 0, 0 ])
...
module create_sphere_test(circle_index) {
  ...
  translate([ pos[0], pos[1], SWATCH_THICKNESS / 2 ]) intersection() {
...
module create_cylinder_hole_test(circle_index) {
  ...
  translate([ pos[0], pos[1], SWATCH_THICKNESS / 2 ]) difference() {
    union() {
