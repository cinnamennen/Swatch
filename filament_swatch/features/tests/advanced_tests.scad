...
module rounded_square_test_pattern_raised(test_count) {
  ...
  if (test_count >= 1)
    translate([
      SWATCH_WALL + TEST_CIRCLE_RADIUS +
          TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING,
  ...
  if (test_count >= 2) {
    ...
    translate([
      SWATCH_WALL + TEST_CIRCLE_RADIUS +
          TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING * 2,
      SWATCH_HEIGHT - SWATCH_WALL - TEST_CIRCLE_RADIUS, SWATCH_THICKNESS / 2
    ]) difference() {
      union() {
      ...
      cylinder(r = r_testhole, h = SWATCH_THICKNESS + TEST_EXTRA_HEIGHT,
               center = true);
    }
  }
  ...
  if (test_count >= 3) {
    translate([
      SWATCH_WALL + TEST_CIRCLE_RADIUS +
          TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING * 3,
      SWATCH_HEIGHT - SWATCH_WALL - TEST_CIRCLE_RADIUS, SWATCH_THICKNESS / 2
  ...
  if (test_count >= 4) {
    ...
    translate([
      SWATCH_WALL + TEST_CIRCLE_RADIUS +
          TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING * 4,
      SWATCH_HEIGHT - SWATCH_WALL - TEST_CIRCLE_RADIUS * 0.5, SWATCH_THICKNESS -
      testradius
    ]) rotate([ 90, 0, 0 ])
  ...
  if (test_count >= 5) {
    translate([
      SWATCH_WALL + TEST_CIRCLE_RADIUS +
          TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING * 5,
      SWATCH_HEIGHT - SWATCH_WALL - TEST_CIRCLE_RADIUS * 2, SWATCH_THICKNESS / 2
    ]) rotate([ -90, 45, 0 ]) cylinder(d1 = SWATCH_THICKNESS * sqrt(2), d2 = 0,
                                       h = SWATCH_THICKNESS, $fn = 4);
    translate([
      SWATCH_WALL + TEST_CIRCLE_RADIUS +
          TEST_CIRCLE_RADIUS * TEST_CIRCLE_SPACING * 5,
      SWATCH_HEIGHT - SWATCH_WALL, SWATCH_THICKNESS / 2
    ]) intersection() {
      rotate([ 90, 0, 0 ]) cylinder(d1 = SWATCH_THICKNESS * sqrt(2), d2 = 0,
                                    h = SWATCH_THICKNESS, $fn = 4);
      cube([ 3 * SWATCH_THICKNESS, 3 * SWATCH_THICKNESS, SWATCH_THICKNESS ],
           center = true);
    }
  }
}